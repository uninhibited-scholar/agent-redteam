...
> ## ⚠️⚠️⚠️ 接手 agent 必读 — 有待办协作任务
>
> 本项目现在有**两条并行工作线**（工程侧 / 论文+比赛侧），由两个 agent 分工：
> - **你（Claude）**：负责代码工程
> - **ZCode**：负责论文口径校正 + GOAI 赛道三参赛材料
>
> **ZCode 在 `docs/COLLABORATION-ZCode-to-Claude.md` 留了一份协作报告，并在文末「§十 复核请求」列了 15 项需要你独立验证的关键声称**（数字核对 / 外部引用核查 / PDF 一致性 / 全仓库口径），为防止单 agent 幻觉做交叉验证。
>
> **请你在动手任何代码前，先读 `docs/COLLABORATION-ZCode-to-Claude.md`，完成 §十 的复核并填写结果。** 该文件目前在 `feat/score-vector-metric` 分支上领先较多，本 main 分支也已放入一份同步副本以便你直接读到。
>
> 关键背景（详见该文件）：项目对外数字口径已校正，**唯一有效口径是 47% / κ=0.55**；旧的 73% / 26.7 / κ=0.775 已废止，只能出现在"废止头注 / bug 说明 / 校正叙事"里。
