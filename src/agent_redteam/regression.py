...
def compare_reports(
    ...
    fixed_keys = sorted(set(baseline_failed) - set(current_failed))
    ...
    findings = [
        ...
        _finding(
            "max_new_critical",
            new_critical <= opts.max_new_critical,
            f"{new_critical} new critical failures vs limit {opts.max_new_critical}",
        ),
        _finding(
            "max_new_high",
            new_high <= opts.max_new_high,
            f"{new_high} new high failures vs limit {opts.max_new_high}",
        ),
    ]
...
def render_regression_terminal(result: RegressionResult) -> str:
    lines = [
        ...
        f"New failures: {result.delta['new_failures']}  Fixed failures: {result.delta['fixed_failures']}",
        "",
    ]
