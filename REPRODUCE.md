...
**本项目把"非文本通道是否比纯文本更脆弱"作为待检验的开放问题，数据给出了明确回答。**

初赛文档把纯文本设为基线（§5 参照系①）、把"通道是否更脆弱"设为待检验问题（§6.1 试跑目标"验证真视觉是否比纯文本更脆弱"）。复赛在 6 模型上跑出了对照数据：

| 核心发现 | 数据 | 含义 |
|---|---|---|
| **纯文本才是最高 bypass 通道** | 6 模型全部：纯文本 bypass（28.8%–86.7%）> 视觉 bypass（0.8%–46.7%） | 证实了初赛文档 §3.3 的"若通道不更脆弱则是有价值负结果"——这就是那个负结果 |
...
73%→47%→κ 修正的过程见[时间线证据链图](validation/figure-correction-chain.svg)——这是"问题定义的有效修正"的活证据（对应评分维度二）。
