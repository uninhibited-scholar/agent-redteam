...
**我们的初始假设被自己的数据推翻了——这是本项目最重要的发现。**

我们带着一个直觉进场：非文本通道（图像、文档管道）应该比纯文本更危险，因为模型对"看不见的文字"防备更弱。**数据证明恰恰相反**：

| 核心发现 | 数据 | 含义 |
|---|---|---|
| **纯文本反而最容易攻破** | 6 模型全部：纯文本 bypass（28.8%–86.7%）> 视觉 bypass（0.8%–46.7%） | "非文本通道更危险"的流行说法不成立 |
| **视觉通道不是均匀盲区** | 6 模型视觉 bypass 跨度 0.8%–46.7% | 强能力依赖，非"模型普遍看不见图像" |
| **"安全"可能只是"没看见"** | minimax-m3 视觉 bypass 仅 1.7%，但 103/120 根本没读到图像 | 低 bypass 可能是 A 层感知失败，不是 C 层防御成功 |

**这构成一个"问题定义的有效修正"**：初始命题"非文本通道是盲区"被修正为"通道之间的差异是能力依赖的，且纯文本才是最高风险通道"。我们把这个演进过程做成了[时间线证据链图](validation/figure-correction-chain.svg)。

**诚实边界（不回避的缺口）**：
- D5 自适应进化（环境设计的第五维）只有 n=2 雏形数据，**统计不充分，不计入任何结论**，列为未来工作
- Judge κ = 0.55（moderate，非 strong），基于截断输入，需更大 N 复审
- 随机标签基线对 1/6 模型（doubao-lite，46.7% 接近随机中心）不适用——这是 **5/6，不是 6/6**

> **关于初赛文档**：初赛文档 §1.1 的立论（"非文本通道会执行"）反映的是**初始假设**，与上述修正后的结论方向不同。这不是隐瞒——恰恰是"问题修正"叙事的一部分：我们带着假设进场，据实修正。
