...
# ===== 精简内容(目标总字数≤3700) =====
SEC_11 = """[草稿·待人工改写] 我们观察到一个被忽视的安全测量缺口：同一条恶意指令，纯文本形式被模型拒绝，但包裹进 SVG/HTML 文档管道、渲染进图像、或拆进多轮对话后，部分模型会执行。对象是商用大模型（GLM、豆包、Kimi、MiniMax、DeepSeek），困扰是"防御在哪一层失效"无法用现有 benchmark 回答。"""

SEC_12 = """(1) OWASP 将提示注入列为 LLM Top 10 头号风险(LLM01)，但评测口径仍以单轮文本为主；(2) OpenAI 2026.07 的 GPT-Red(间接提示注入对 GPT-5.1 达 84%、人类仅 13%)证明该方向是前沿焦点，但闭源、不测他厂、无可复现基线；(3) 学术工作(arXiv:2509.05883 等)各自只覆盖单一通道。无人在同一模型做跨通道系统对照，更无人区分"模型没看见"与"模型被攻破"。"""

SEC_13 = """[草稿·待人工改写] 价值：跨通道盲区若存在，意味着单轮文本 benchmark 上"安全"的模型可能在另一通道失守，主流评测系统性低估风险。
[草稿·待人工改写] AI 介入理由：组合空间 N×M×K×L×多模型人工无法穷举。Agent 的价值是(1)系统可审计扫过空间并留日志；(2)做结构化拆解(A/B/C，见§三)而非只记 pass/fail；(3)以成功 bypass 为种子跨通道进化。"""
...
SEC_22 = """观察：每次调用的输入(含载体形态)、模型输出、A/B/C 判定。行动维度：D1 单轮文本(基线)、D2 变异(11 策略)、D3 多轮对话、D4 通道载体(纯文本/文档管道/真视觉)、D5 自适应进化(初步)。
...
SEC_31 = """[草稿·待人工改写] 五类发现信号(含负结果与问题修正)：①正向——某通道 bypass 显著高于基线(如真视觉 doubao-lite 46.7%)；②反例——某通道比纯文本更难攻(文档管道 bypass 更低)；③稳定负结果——分层注入不放大 bypass；④失败模式——评分器假阳性(length 规则致虚高，已校正)；⑤问题修正——初测 73% 校正为 47%。判据：所有比率附 Wilson 95% CI，发现需≥2 模型复现。"""

SEC_32 = """四层参照(比 bypass 率带 CI)：①平凡基线——纯文本直注(D1)作下限；②载体对照——同 payload 纯文本/文档/视觉只变载体，隔离通道效应；③跨模型——同环境跑 8 模型，多模型复现=共性；④评分器自审计——受控重跑测假阳性。判定超参照：某通道 CI 下限>基线 CI 上限且方向≥2 模型一致。"""
...
SEC_41 = """目标：验证真视觉是否比纯文本更脆弱，测 6 模型 bypass 范围。输入：14 套件种子，每模型 N=120。步骤：固定种子生成 payload→渲染图像送 6 模型→金丝雀判定 A/B/C→算 bypass+CI→与基线对照。
输出(已实测)：bypass 从 0.8%(doubao-code)到 46.7%(doubao-lite)跨 6 模型，强能力依赖而非均匀盲区；A 层(没读到图)从 doubao-pro 26/120 到 minimax 103/120。日志存 validation/expB-render-{model}.json。"""
