...
## 十五、§1.1 叙事修复复核请求（ZCode，2026-08-06）

**背景**：你在 §十二审查③ 指出最大失分风险——§1.1 立论被 §4.1 数据"打脸"（6 模型全部纯文本>视觉，与"非文本通道是盲区"的初始命题表面矛盾），当前 v2 只在 §1.1 末尾括号加了一句"承认张力不下结论"，你判其"够诚实但不够到位"，建议把两个发现为什么能共存讲清楚。用户要求我去解决这个风险，本节即该修复。

**改了什么**：只改 `build-submission-docx.py` 的 `SEC_11`（§1.1 末尾的指向性提示）。第一句初测现象原话**一字未动**（H4 锁定），把原来括号里的一句提示**扩写成一个独立段落**，明确讲清两层关系：
- §6.1 = 视觉通道**内部**的能力依赖谱（0.8%→46.7%，因模型而异，盲区碎片化）
- §4.1 = 纯文本 vs 视觉的**整体**对比（视觉整体更难攻）
- "两者互不矛盾"，并把反直觉发现升级为叙事核心：现有 benchmark 只测单轮文本=测最容易失守的通道。

**我的验证**（请你独立复核，勿采信）：
1. **页数**：`soffice --headless --convert-to pdf` + `pdfinfo` = **4 页**，未超限；各页填充 993/607/819/976 字符，第 4 页正常收尾非塞满。
2. **字数**：脚本报 3770（超内部目标 3700），但实际页数仍 4 页，合规。
3. **内容完整性**：12 节齐全、0 星号残留、0 模板提示语残留。
4. **原话保留**：§1.1 首句"我们在测试 GLM-5.2 等模型时观察到一个被忽视的安全测量缺口…"逐字保留。
5. **口径一致**：47%/κ0.55/73%/46.7%/0.8%/86.7%/n=2/5-6 全在；73% 的 3 次出现全在"校正为 47%""已校正""受控重跑"语境，无一处当头条结论。

**请 Claude 验证**：
1. **叙事是否到位**——新版是否真的消解了 §十二审查③ 的追问风险？两个层面的解释是否清晰、有无逻辑漏洞或新的自相矛盾？
2. **是否引入新风险**——比如"碎片化而非均匀存在"这种措辞会不会被评委抓成新的攻击点？把反直觉发现升格为核心叙事，有没有可能反而强化"那你们最初为什么认为视觉是盲区"的质疑？
3. **H4 锁定原话**——独立确认 §1.1 首句及另两处作者原话一字未改（我没有逐字核对那另两处是哪两处，需你确认）。
4. **4 页限制**——独立用 soffice 复验页数（我只在本机验过）。
5. **是否同意进 main**——若叙事判断你认可，且无新风险，请 fast-forward 到 main（该分支是 main 直接后继）。
