...
CLI · TUI · Web Dashboard | 2,319 测试样本 · 14 攻击套件 · OWASP LLM 2025 十类均有测试
...
| 套件 | 测什么 | 样本量 | OWASP |
|------|--------|--------|-------|
| **injection** | Prompt 注入能否诱导 agent 执行危险操作（含 40 条 allow controls） | 255 | LLM01 |
| **tool_abuse** | 破坏性工具调用（rm -rf / DROP TABLE / PowerShell）能否被拦截（含 40 条 allow controls） | 243 | LLM01 |
...
**总计 2,319 条测试样本**，OWASP LLM Top 10 2025 十个分类均有代表性测试；分类覆盖不等同于覆盖深度充分，
且 injection/tool_abuse 明确包含 allow controls（以 `agent-redteam list --format json --validate` 为准）。详见 [风险分类法](taxonomy.md)。
...
| 测试 | 240 个全绿 |
| 测试样本 | 2,319 条（14 套件，含 benign controls） |
