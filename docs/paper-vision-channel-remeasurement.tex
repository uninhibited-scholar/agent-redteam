...
\subsection{Other channels: mutation and multi-turn findings (inherited, \emph{not} re-measured here)}

This paper's controlled re-measurement (\S4.1--4.6) focuses on the \textbf{vision channel} and the document-pipeline text channel. To avoid giving the impression that the only findings in this project concern vision, we report here two \emph{inherited} findings about \textbf{other channels} --- text mutation (D2) and multi-turn dialogue (D3) --- drawn from the same superseded preliminary study whose 73\% vision headline \S4.1 corrects. \textbf{These two findings are reported in their original, unrevised form; they were not re-measured under this paper's canary / A/B-C / independent-judge methodology, and they should be read as candidate signals worth scaling up with that methodology, not as validated results of this paper.} Confidence intervals below were computed \emph{post hoc} (Wilson score intervals, from the published $k/n$) and were not part of the original study's reporting; retrospective CI computation on a published point estimate is not equivalent to a pre-registered interval.

\textbf{(a) Mutation attacks: a ``structure-vs-surface'' regularity (GLM-5.2, 11 strategies $\times$ 5 samples = 55 trials).} When the same malicious instruction was paraphrased through one of 11 mutation strategies, the overall bypass rate was 19/55 = \textbf{35.2\%} (95\% CI [23.4, 47.7], Wilson). The informative signal was not the aggregate rate but the \textbf{spread across strategy types}: strategies that alter the instruction's \emph{structure} (reframe / multilingual-mix / split-reassembly) each bypassed at \textbf{3/5 = 60\%} (95\% CI [23.1, 88.2] --- very wide at $n{=}5$), while a strategy that alters only the \emph{surface} (case-spoofing) bypassed at \textbf{0/5 = 0\%} (95\% CI [0.0, 43.4]). The qualitative contrast (structure-altering $\gg$ surface-altering) is the candidate finding; at $n{=}5$ per strategy the CIs are too wide for the per-strategy rates to be taken as precise. If it replicates under this paper's scoring, the regularity would suggest the model's text-channel defense may depend on surface pattern matching --- a text-channel analogue to \S4.1's scorer-vs-behavior distinction.

\textbf{(b) Multi-turn attacks: single-turn evaluation may overestimate small-model safety.} On GLM-5.2, a multi-turn attack script reached \textbf{11/50 = 22\%} bypass (95\% CI [12.8, 35.2], Wilson), versus this model's 0\% single-turn text-injection baseline. On the much smaller qwen2.5:0.5b, single-turn injection succeeded at 5/5 = 100\% but a multi-turn script dropped to 3/5 = 66\% (95\% CI [23.1, 88.2]); the direction of effect (multi-turn \emph{worse} for the strong model, multi-turn \emph{better} for the weak one) is the candidate signal, not the precise rates. The qwen2.5 direction is the less expected of the two and is the least-supported point in this subsection.

\textbf{Why these are reported here despite their weaker status.} They are the project's findings about \emph{non-vision} channels, and a paper titled toward cross-channel injection that reports only vision would over-narrow its scope. Reporting them with explicit, visible caveats --- small N, single-model, no A/B-C decomposition, retrospective CI, not judge-audited --- is more honest than either presenting them at parity with \S4.1--4.6 or silently dropping them. The right next step for each is to re-run it under this paper's full methodology (canary scoring, $\geq$6 models, N=120, A/B-C perception decomposition, independent-judge $\kappa$). Until that is done they are \textbf{hypotheses generated by the preliminary study}, not measurements established by this one.
