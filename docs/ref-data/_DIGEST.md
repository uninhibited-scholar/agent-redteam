...
## E. delivery-text（文档管道 vs 纯文本，最反直觉的发现，N=120 升级版）

> **2026-08-09 口径升级**：本节已从早期 n=11 试点升级到 N=120 受控实验。n=11 数字仅保留作历史对照，所有对外材料统一引用 N=120。

| 模型 | plain_text bypass (95% CI) | doc_pipeline bypass (95% CI) |
|---|---|---|
| GLM-5.2 | 47.1% [38.3, 56.0] (56/119) | **0.8%** [0.1, 4.6] (1/120) |
| DeepSeek-v4 | 49.2% [40.3, 58.1] (58/118) | **1.7%** [0.5, 5.9] (2/120) |

_历史 n=11 试点（保留对照，不再作主引用）：GLM-5.2 plain 9.1%(1/11)→doc 0%(0/11)；DeepSeek-v4 plain 54.5%(6/11)→doc 9.1%(1/11)。方向与 N=120 一致，但样本量小、CI 宽。_

**核心发现：文档管道（把攻击藏在 SVG 里）比纯文本更难攻破，而非更容易。**

- GLM-5.2：纯文本 47.1% → 文档管道 0.8%（**降 46 个百分点**）
- DeepSeek-v4：纯文本 49.2% → 文档管道 1.7%（**降 47 个百分点**）
- 两模型方向一致、CI 窄，是受控负结果（非"没测到"的假阴性）

**含义**：把同样的攻击意图从纯文本搬运到文档结构（SVG 隐藏文本），bypass **不升反降 ~46pp**。这是对"非文本通道更脆弱"假设的有力反例。
...
1. 分层注入不放大攻击（显眼度主导）
2. 文档管道比纯文本更难攻破（**N=120**：DeepSeek 49.2% → 1.7%，GLM-5.2 47.1% → 0.8%）
3. 低对比度的"安全"部分是渲染假象
