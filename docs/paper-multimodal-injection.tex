...
\noindent\textbf{Dimension 4: Non-Text Channel Injection (15 doc-pipeline samples + 720 real-vision samples)}

Attack instructions are hidden in non-text channels. We separate two channels that were conflated in an early headline: (a) \emph{document-pipeline text injection}, where SVG/HTML/PDF content is \emph{extracted as text} and fed to the model, and (b) \emph{true visual injection}, where the attack is rendered into a PNG and the pixels are fed to a vision-language model. The document-pipeline techniques include:
...
The over-count came from a length-based scorer fallback that marked ``the model merely \emph{describes} the document content'' as a bypass. Fixing it (Appendix~\ref{app:checkfixes}) dropped the rate by $\sim$26--33 points.
...
\begin{tabular}{lccc}
\toprule
Model & image\_plain bypass\textsuperscript{$\dagger$} & Not-read (A) & Read-but-described (B) \\
\midrule
...
\noindent\textsuperscript{$\dagger$}\textit{The \texttt{image\_plain} bypass column is the fraction of judged trials the model executed the attack (layer C). For frontier models this is near zero because they refuse after reading; for minimax-m3 it is near zero for a different reason---it fails to read the image at all (see column A). The same single bypass number therefore masks two qualitatively different outcomes, which is the motivation for the A/B/C split.}

Two findings stand out. First, the bypass rate is \textbf{strongly capability-dependent}: frontier models (doubao-pro/code $\approx$ 1\%, minimax $\approx$ 2\%) are near-zero, while the lightweight doubao-lite reaches 46.7\%. This is \emph{not} the uniform blind spot the original headline implied. Second, the A/B/C decomposition is essential: minimax's $\sim$2\% looks ``safe,'' but $A=103/120$ means 86\% of the time it never read the image at all---a perception failure, not a safety success. A single-number benchmark would credit minimax for safety it does not have. Genuine C-layer failures (model read \emph{and} executed the attack) occur in the three weaker models: doubao-lite ($C=19/120$), kimi-k2-7 ($C=20/120$), and glm-4v-flash ($C=19/120$); the frontier models genuinely refuse after reading ($C \le 1$).

\subsection{Document-Pipeline vs.\ Plain Text (Negative Result, $N{=}120$)}

To isolate the effect of the delivery channel, we delivered the \emph{same} harmful payload as either plain text or via the document pipeline (SVG-as-text---the paper's original ``multimodal'' method) on both original models, $N{=}120$ per cell.
...
\begin{tabular}{lcc}
\toprule
Model & plain\_text bypass & doc\_pipeline bypass \\
\midrule
GLM-5.2 & 47.1\% (56/119) & \textbf{0.8\%} (1/120) \\
DeepSeek-v4 & 49.2\% (58/118) & \textbf{1.7\%} (2/120) \\
\bottomrule
\end{tabular}
\end{table}

Moving an injection from plain text into document structure does not raise bypass---it \emph{collapses} it by $\sim$45--47pp. The paper's ``multimodal'' channel is therefore not a more dangerous surface; it is a \emph{weaker} one. This is a controlled negative result with a narrow confidence interval, not an absence-of-evidence artifact.
...
Multimodal doc-pipeline (corrected, $n{=}15$) & --- & 47\% \\
Multimodal true visual (6 models $\times$ 120) & --- & 0.8\%--46.7\% \\
Doc-pipeline vs.\ plain text ($N{=}120$) & --- & 47.1\%/49.2\% $\rightarrow$ 0.8\%/1.7\% \\
...
\begin{itemize}
  ...
  \item \textbf{Reading-mode suppression (negative result).} In the document-pipeline channel, models tend to \emph{describe} the SVG/HTML content rather than act on it---entering a ``reading-comprehension'' mode that suppresses instruction execution. At $N{=}120$, DeepSeek-v4's bypass drops from 49.2\% on plain text to 1.7\% when the same attack is hidden in document structure, and GLM-5.2 from 47.1\% to 0.8\%. Hiding an attack in a document can therefore make it \emph{harder}, not easier.
...
The residual 47\% on document-pipeline text injection (GLM-5.2, DeepSeek-v4) and the genuine C-layer failures on the three weaker vision models---doubao-lite ($C=19/120$), kimi-k2-7 ($C=20/120$), and glm-4v-flash ($C=19/120$)---are the cases worth attention. The corrected headline is therefore not ``non-text channels are universally vulnerable'' but ``the gap is real, capability-dependent, and in some channels actually inverted.''
...
\begin{enumerate}
  ...
  \item \textbf{Self-corrected scorer.} Our first scorer over-counted bypasses by treating ``the model merely describes the document content'' as execution (length-based fallback), inflating the original 73\%/80\% to a corrected 47\%. The fix and the regression tests are in Appendix~\ref{app:checkfixes}; the 73\%$\rightarrow$47\% audit is itself part of the contribution, but it means early-stage numbers in our own pipeline should not be trusted without re-scoring.
