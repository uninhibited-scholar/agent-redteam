...
def test_sbom_summarizes_python_npm_and_artifacts(tmp_path):
    ...
    artifact = root / "dist" / "agent_redteam-0.5.0.tar.gz"
...
def test_release_manifest_summarizes_artifacts_evidence_and_git(tmp_path):
    ...
    dist.mkdir()
    validation.mkdir()
    wheel = dist / "agent_redteam-0.5.0-py3-none-any.whl"
    sdist = dist / "agent_redteam-0.5.0.tar.gz"
...
def test_release_manifest_redacts_unavailable_evidence_reason(tmp_path):
    ...
    dist = root / "dist"
    dist.mkdir()
    (dist / "agent_redteam-0.5.0-py3-none-any.whl").write_text("wheel", encoding="utf-8")
    (dist / "agent_redteam-0.5.0.tar.gz").write_text("sdist", encoding="utf-8")
...
def test_release_gate_passes_with_fake_runner_and_artifacts(tmp_path):
    ...
    assert "2322 samples" in sample_audit.detail
    assert "PASS" in markdown
...
def test_doctor_detects_action_key_argv_pattern(tmp_path):
    ...
    (root / "README.md").write_text("uses: uninhibited-scholar/agent-redteam@v0.5.0\n", encoding="utf-8")
...
def test_action_uses_env_key_not_argv():
    ...
    assert '--max-attempts "$INPUT_MAX_ATTEMPTS"' in text
