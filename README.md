...
| 🟫 **Supply Chain** | typosquatting / dependency confusion / CI-CD 投毒 | 215 | LLM02 |
...
| 🔘 **Over-Dependency** | 幻觉/过度自信——对不确定的事给了确定答案 | 215 | LLM10 |
| ⚫ **Multi-Turn** | 多轮建立信任/上下文后的渐进攻击 | 50 | LLM01 |

**总计 2,184 条攻击样本**，所有数字以 `agent-redteam list --format json --validate` 的实际输出为准。
当前内置 catalog 覆盖 LLM01、02、04-10；LLM03（供应链模型/数据污染）尚未设为独立 suite，不将其宣传为已覆盖。
