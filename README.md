...
| 跨通道梯度（核心，已校正） | 文本 100 → 文档管道 26.7（原始口径）→ 校正后 47% → 真实视觉 0.8%~46.7% | 对齐盲区真实存在但强烈依赖模型能力，非普遍塌陷 | ✅见下 |
| 感知/合规拆解（方法贡献） | 6 模型 A/B/C 拆解，某模型 86% 属"根本没看懂攻击" | 低 bypass 可能是"真安全"也可能是"读不懂"，两者需分开判定 | ✅已验证 |
