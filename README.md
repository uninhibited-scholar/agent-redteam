...
CLI · TUI · Web Dashboard | 2,204 测试样本 · 12 攻击套件 · 覆盖 9 个 OWASP LLM 2025 风险分类
...
AI agent 在没有安全检查的情况下就被发布——不像普通软件有 `npm audit`、`snyk scan` 这样的工具。**Agent Redteam 就是这个缺失的等效工具**：在 agent 上线前，系统性地从 12 个测试套件跑 2,204 条攻击测试，输出具体的 pass/fail 分数，而不是模糊的"感觉安全"。
...
**总计 2,204 条攻击样本**，所有数字以 `agent-redteam list --format json --validate` 的实际输出为准。
当前内置 catalog 覆盖 LLM01-07、09、10；LLM08（向量/嵌入弱点）尚未设为独立 suite。完整映射见 [风险分类法](docs/taxonomy.md)。
